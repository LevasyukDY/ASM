...
int main()
{
	...
	// TODO: Неправильно работает FSUB, надо разобраться в какие регистры надо заносить значения
	...
	__asm {
		...
		// Цикл подсчёта периметра (суммы длин) треугольника
			...
			fst   ST(1)			 // Копирует число из ST(0) в ST(1)

		LOOP_SUM:
			fld   [arrL+edi]		// Помещаем в ST(0) поочерёдно эл-ты массива arrL
            fadd  ST(1), ST(0)		// Сложение ST(1) = ST(1) + ST(0)
            add   edi, 4			// Увеличиваем индекс на 4
		...
		CalcLength:
			...
			fsub  ST, ST(1)			// Вычитание первого эл-та из второго ST(0) = ST(0) - ST(1)
			fmul  ST(0), ST(0)		// Возведение в квадрат
			fst   ST(4)				// Копирует число из ST(0) в ST(3)
			fst   [x]
			...
			// sqrt(x + y)
			fld		[x]			// Меняет местами ST(3) и ST(0)
			fst     ST(2)
			fld     [y]
			fadd	ST(2), ST		// Сложение ST(0) = ST(0) + ST(2)
			fxch    ST(2)
			fst   [len]
